...
Hashtable	线程安全，线程安全，多线程还能有正确的结果。vector 线程安全，相当ArrayList于加了一个锁。
stack 线程安全的栈。
...
LinkedList list = new LinkedList();// 能添加，也能获取
...
编译时两个String 常量（"aaa"+"bbb"）相加，会直接优化成目标常量，两个字符串变量(a+b)相加，相当于new StringBuilder
...
6、如果两个操作数其中有一个是double类型，另一个操作就会转换为double类型。

7、否则，如果其中一个操作数是float类型，另一个将会转换为float类型。

8、否则，如果其中一个操作数是long类型，另一个会转换为long类型。
...
Math.floor(-8.5)=(double)-9.0 // 返回true
...
& 与运算： 左右两个逻辑值都为true时，整个表达式的值为true；一方为false ，其结果为false 。

| 或运算 ： 左右两边，只要有一个表达式为true，其结果为true； 都为false时，其结果为false。
